...
class pyramid_images_generator():
        
    def __init__(self):
         self.gaussian_imagesSet = []
         self.gaussian_kernels =[]

    def generateBaseImage(self,src: array, sigma: float, assumed_blur: float) -> array:
    ...
    def computeNumberOfOctaves(self,src_shape: tuple) -> int:
        """
    ...
    def generateGaussianKernels(self,sigma, num_intervals):
        ...
        k = 2 ** (1. / num_intervals)
        self.gaussian_kernels = zeros(num_images_per_octave)
        self.gaussian_kernels[0] = sigma

        for image_index in range(1, num_images_per_octave):
            sigma_previous = (k ** (image_index - 1)) * sigma
            sigma_total = k * sigma_previous
            self.gaussian_kernels[image_index] = sqrt(sigma_total ** 2 - sigma_previous ** 2)
        return self.gaussian_kernels
    
    def generateGaussianImages(self,src: array, num_octaves: int, gaussian_kernels: np.ndarray) -> array:
        """
        ...
        for octave_index in range(num_octaves):
            ...
            self.gaussian_imagesSet.append(gaussian_images_in_octave)
            octave_base = gaussian_images_in_octave[-3]
            src = resize(octave_base, (int(octave_base.shape[1] / 2), int(octave_base.shape[0] / 2)),
                        interpolation=INTER_NEAREST)
        return array(self.gaussian_imagesSet, dtype=object)



class key_points_generator():

    def generateDoGImages(self,gaussian_images: array) -> array:
        """
    ...
    def findScaleSpaceExtrema(self,gaussian_images: array, dog_images: array, num_intervals: int, sigma: float,
            ...
            for image_index, (first_image, second_image, third_image) \
                ...
                for i in range(image_border_width, first_image.shape[0] - image_border_width):
                    for j in range(image_border_width, first_image.shape[1] - image_border_width):
                        ...
                        if self.isPixelAnExtremum(first_image[i - 1:i + 2, j - 1:j + 2], second_image[i - 1:i + 2, j - 1:j + 2],
    ...
    def isPixelAnExtremum(self,first_subimage: array, second_subimage: array, third_subimage: array, threshold: float) -> bool:
        """
...
def Sift(src: array, sigma: [float, int] = 1.6, num_intervals: int = 3, assumed_blur: float = 0.5,
    ...
    """
    pyramid_images = pyramid_images_generator()
    key_point=key_points_generator ()
    source = copy(src)
    source = cvtColor(source, COLOR_BGR2GRAY)
    source = source.astype('float32')
    base_image = pyramid_images.generateBaseImage(source, sigma, assumed_blur)
    num_octaves = pyramid_images.computeNumberOfOctaves(base_image.shape)
    gaussian_kernels = pyramid_images.generateGaussianKernels(sigma, num_intervals)
    gaussian_images = pyramid_images.generateGaussianImages(base_image, num_octaves, gaussian_kernels)
    dog_images = key_point.generateDoGImages(gaussian_images)
    keypoints = key_point.findScaleSpaceExtrema(gaussian_images, dog_images, num_intervals, sigma, image_border_width)
    keypoints = key_point.removeDuplicateKeypoints(keypoints)
    keypoints = key_point.convertKeypointsToInputImageSize(keypoints)
    descriptors = key_point.generateDescriptors(keypoints, gaussian_images)
