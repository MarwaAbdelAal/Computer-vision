...
class KMeans:
    ...
    def predict(self, X):
        if (self.K < 2) or (self.max_iters < 1) :
            pass
...
if __name__ == "__main__":
    ...
    segmented_image, labels = apply_k_means(img_luv)
    end_time = time.time()

    Kmeans_time = format(end_time - start_time, '.5f')
    print(f'K-means Computation Time = {Kmeans_time} sec')    # print(  end_time - start_time)
